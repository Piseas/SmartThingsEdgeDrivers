...
-- Helper: replicate the driver's dewpoint calculation function.
...
local mock_device = test.mock_device.build_test_zigbee_device(
  {
    profile = t_utils.get_profile_definition("humidity-temp-battery-dewpoint.yml"),
    zigbee_endpoints = {
      [1] = {
        id = 1,
  ...
  "Humidity report should be handled",
  {
    ...
    },
    {
      ...
      message = mock_device:generate_test_message("main",
        capabilities.relativeHumidityMeasurement.humidity({ value = 79.0, unit="%" }))
    }
  }
)
...
test.register_message_test(
  "Humidity report should be handled for 0 value",
  {
    ...
    },
    {
      ...
      direction = "send",
      message = mock_device:generate_test_message("main",
        capabilities.relativeHumidityMeasurement.humidity({ value = 0.0, unit ="%" }))
    }
  }
)
...
test.register_message_test(
  "Humidity report should be handled for 100 value",
  {
    ...
    },
    {
      ...
      direction = "send",
      message = mock_device:generate_test_message("main",
        capabilities.relativeHumidityMeasurement.humidity({ value = 100.0, unit = "%" }))
    }
  }
)
...
test.register_coroutine_test(
  "Handle tempOffset preference - corrected to check for adjusted result",
  function()
...
test.register_coroutine_test(
  "Handle humidityOffset preference - corrected to check for adjusted result",
  function()
    test.socket.environment_update:__queue_receive({ "zigbee",
      { hub_zigbee_id = base64.encode(zigbee_test_utils.mock_hub_eui) } })
...
test.register_coroutine_test(
  "Dewpoint loop test: temperature drop, rise, and RH preference handling",
  function()
    -- STEP 1: Set RH preference to 20
    test.socket.device_lifecycle:__queue_receive(mock_device:generate_info_changed({ preferences = { rhThreshold = 20 } }))
    test.wait_for_events()
    
    -- STEP 2: Initialize temperature and humidity
    test.socket.zigbee:__queue_receive(
      {
        mock_device.id,
        RelativeHumidity.attributes.MeasuredValue:build_test_attr_report(mock_device, 7000)
      }
    )
    test.socket.capability:__expect_send(mock_device:generate_test_message("main",
      capabilities.relativeHumidityMeasurement.humidity({ value = 70.0, unit = "%" })))
    
    test.socket.zigbee:__queue_receive(
      {
        mock_device.id,
        TemperatureMeasurement.attributes.MeasuredValue:build_test_attr_report(mock_device, 3200) -- Converted 90F to 32C
      }
    )
    test.socket.capability:__expect_send(mock_device:generate_test_message("main",
      capabilities.temperatureMeasurement.temperature({ value = 32.0, unit = "C" })))
    
    local dewpoint_initial = calculate_dew_point(32, 70)
    test.socket.capability:__expect_send(mock_device:generate_test_message("main",
      capabilities.dewPoint.dewpoint({ value = dewpoint_initial, unit = "C" })))
    
    -- STEP 3: Simulate temperature drop to 21°C (Converted 70F to 21C)
    for temp = 32, 21, -1 do
      test.socket.zigbee:__queue_receive(
        {
          mock_device.id,
          TemperatureMeasurement.attributes.MeasuredValue:build_test_attr_report(mock_device, temp * 100)
        }
      )
      test.socket.capability:__expect_send(mock_device:generate_test_message("main",
        capabilities.temperatureMeasurement.temperature({ value = temp * 1.0, unit = "C" })))

        local last_switch = mock_device:get_latest_state("main", capabilities.switch.ID, capabilities.switch.switch.NAME) or "off"

        if temp < (dewpoint_initial + 1.0) and (last_switch == "off") then
    ...
    -- STEP 4: Simulate temperature rise back to 32°C
    for temp = 21, 32, 1 do
      test.socket.zigbee:__queue_receive(
        {
          mock_device.id,
      ...
      test.socket.capability:__expect_send(mock_device:generate_test_message("main",
        ...
        local last_switch = mock_device:get_latest_state("main", capabilities.switch.ID, capabilities.switch.switch.NAME) or "on"
      if temp > (dewpoint_initial + 1.0) and (last_switch == "on") then
